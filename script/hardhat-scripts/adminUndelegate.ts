...
async function main() {
  ...
  console.log("trying to undelegate");
  console.log("current operator is", currentOperator);

  const undelegateCalldata = nodeDelegator.interface.encodeFunctionData("undelegate");
  await multisig.submitTransaction(nodeDelegatorAddress, 0, undelegateCalldata);
